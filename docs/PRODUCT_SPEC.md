...
| Auth/security | Password reset, login alert, OTP: 35 first distinct month, +10 each additional | 55 |
...
> **Amended 2026-07-15 (auth).** The row read "35 first, +10 for a second distinct month", cap 45, which discarded the month count past the second and put a ceiling under the family below the `high` threshold. The table therefore asserted that no quantity of password-reset evidence, over any span, can make us confident an account exists. That is backwards: the service honoured the reset, which is the service confirming the account is there and that this user holds it, and unlike a signup mail it keeps arriving for accounts older than the mailbox. Measured against a real 811-message scan, this was the whole shape of the result: exactly one candidate reached `high`, while GitHub (252 messages) sat at 65, Vercel (47) at 60, and both were capped by this row. Auth stays below `high` alone at 55 on purpose, because an OTP can reach someone with no account (guest checkout, phone verification); it now needs corroboration rather than a signup mail that no longer exists.
>
> **Amended 2026-07-15 (transaction).** This table previously read "10 per distinct month; recurring renewals in >= 3 distinct months floor the family at 25", and the example scored that subscription 25. The two could not both hold, and the floor could never bind in the first place: three distinct months already scores 30, so a floor of 25 is unreachable. `score.js` had resolved the contradiction the other way, treating 25 as a ceiling, which made this table's own cap of 30 dead and left the family scoring only {0, 10, 20, 25}. That cost real candidates a band: welcome + receipts in four distinct months scored 65 `review` where the table says 70 `high`. The table now governs and the example's arithmetic moved 40 -> 45; its actual claim is unchanged, and transaction still cannot reach `high` without signup or auth (30 + 15 + marketing 5 = 50).
...
**What this widened:** GA needs the network, so shipping it added `googletagmanager.com` to `script-src` and `img-src` and the two `*.google-analytics.com` / `*.analytics.google.com` beacon origins to `connect-src` (`server/server.js`). §6 went on describing the pre-GA allowlist for a day, which is the more dangerous half of the same mistake: a reader checking whether the browser can phone Google would have concluded it cannot.
